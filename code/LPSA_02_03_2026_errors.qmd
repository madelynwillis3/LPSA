...
# Safety: ensure we have numeric sand/silt/clay in percent units
dat <- lpsa2 %>%
...
# Pairwise table for ALL base_IDs
pairwise_all <- dat %>%
  group_by(base_ID) %>%
  group_modify(\(d, key) {
    ...
    tibble(
      ...
      # your 5% rescue rule: sand AND clay within tolerance
      pass_5pct = (sand_diff <= tol) & (clay_diff <= tol),

      # total error for "lowest error combo"
      total_error = sand_diff + silt_diff + clay_diff
    )
  }) %>%
  ungroup()

# Base-level chooser + averaged outputs
avg_best2_by_base <- dat %>%
  group_by(base_ID) %>%
  group_modify(\(d, key) {

    # If only 1 sample, we can't average a pair
    if (nrow(d) < 2) {
      return(tibble(
        n_samples = nrow(d),
    ...
    # helper to pick best row from pw (lowest error)
    ...
    if (all_same_class) {
      # all same class: choose lowest error pair overall
      chosen <- pick_best(pw)
      pick_status <- "ALL_CLASS_AGREE"

    } else if (modal_n >= 2) {
      # class OK: choose lowest-error pair within modal class
      chosen <- pw %>%
        filter(same_class, class_1 == modal_class) %>%
        { if (nrow(.) == 0) pw %>% filter(same_class) else . } %>%   # fallback: any same-class
        pick_best()
      pick_status <- "CLASS_OK"

    } else {
      # class disagreement: try 5% rescue
    ...
    # compute averages for chosen IDs
    id1 <- chosen$ID_1[[1]]
    id2 <- chosen$ID_2[[1]]

    r1 <- d %>% filter(ID == id1) %>% dplyr::slice(1)
    r2 <- d %>% filter(ID == id2) %>% dplyr::slice(1)
    ...
    tibble(
      ...
      avg_SAND = mean(c(r1$SAND, r2$SAND), na.rm = TRUE),
      avg_SILT = mean(c(r1$SILT, r2$SILT), na.rm = TRUE),
      avg_CLAY = mean(c(r1$CLAY, r2$CLAY), na.rm = TRUE),
...
final_output <- avg_best2_by_base %>%
  select(
    ...
    lab_texture_class = picked_class
  )
final_output

...
results_df <- list.files("F:/Willis/git/LPSA/data_do_not_push/LPSA data", pattern = "\\.csv$", recursive = TRUE, full.names = TRUE, ignore.case = TRUE
) %>%
...
```{r Detect high dilution samples, warning = FALSE, eval=FALSE}
#do not run:
...
# STEP 1: Filter high-clay samples
filtered_results <- results_df %>%
  filter(CLAY < 80)  # Exclude samples with CLAY >= 80%

# STEP 2: Group by `base_ID` and compute average values and replicate counts
...
# STEP 3: Compute USDA texture classes on the averaged data
# Prepare data for texture classification
selected_data <- averaged_results %>%
  select(CLAY = avg_clay, SILT = avg_silt, SAND = avg_sand) %>%
  as.data.frame()

# Classify texture points
texture_classes <- TT.points.in.classes(
  tri.data = selected_data,
  class.sys = "USDA.TT"
)

# Optional: Plot the texture classes
TT.plot(
  tri.data = selected_data,
  class.sys = "USDA.TT",
  col = "black",
  pch = 19,
  cex = 0.5,
  main = "Texture Classes for All Averaged Soil Samples"
)

# Extract primary and boundary texture info
texture_info <- apply(texture_classes, 1, function(x) {
  max_val <- max(x)
  classes <- colnames(texture_classes)[which(x == max_val)]
  list(
...
# STEP 4: Add texture classification info to the averaged results
...
# STEP 5: Map USDA texture names to simplified texture classes
averaged_results <- averaged_results %>%
  mutate(
    texture_class = case_when(
      primary_texture == "SaClLo" ~ "SCL",
      ...
      primary_texture == "SiLo" ~ "SIL",
      primary_texture == "Sa" ~ "S",
      TRUE ~ as.character(primary_texture)  # Default to the USDA name if not mapped
    )
  )

# STEP 6: Output summarized results with texture classifications
cat("\nAveraged Results with Texture Classes:\n")
print(averaged_results)

# Save the averaged results as a CSV file
today <- format(Sys.Date(), "%m_%d_%Y")
filename_summary <- paste0("../output_do_not_push/averaged_results_texture_", today, ".csv")
write.csv(averaged_results, filename_summary, row.names = FALSE)

# Optional: Generate a styled table for HTML
results_table <- averaged_results %>%
  select(base_ID, avg_clay, avg_silt, avg_sand, texture_class, num_replicates, is_boundary, boundary_classes) %>%
  kable(caption = "Averaged Texture Classifications for All Soil Samples", format = "html") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive")) %>%
  scroll_box(height = "400px")
...
# Save per-sample output too, if needed
filename_samples <- paste0("../output_do_not_push/all_results_texture_", today, ".csv")
write.csv(filtered_results, filename_samples, row.names = FALSE)
cat("\nFinal per-sample results saved to:", filename_samples, "\n")
